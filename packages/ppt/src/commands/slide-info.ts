 ...
 * get_slide_info with the slide_index.
 * Test: Mock the client, call slideInfoCommand(["/tmp/x.pptx", "0"]), assert callTool
 * called with "get_slide_info" and { slide_index: 0 }. No save_presentation call.
 */
...
export async function slideInfoCommand(args: string[]): Promise<unknown> {
	...
	return withOpenReadonly(file, async (client) => {
		return client.callTool("get_slide_info", { slide_index: slideIndex });
	});
}
