...
export async function mailGetCommand(args: string[]): Promise<unknown> {
  ...
  if (!id) {
    throw new AxiError("message id is required", "VALIDATION_ERROR", ["outlook-axi mail-get <id>"]);
  }
...
export async function mailSendCommand(args: string[]): Promise<unknown> {
  ...
  if (!to || !subject || !body) {
    throw new AxiError("to, subject and body are required", "VALIDATION_ERROR", [
